...
class QPINN(ForwardIVP):
    ...
    def __init__(
        self, config, psi_real0, psi_imag0, t_star, x_star, hbar, mass, Tcar, Xcar
    ):
    ...
    def r_net(self, params, t, x):
        ...
        H_psi_real = -0.5 * (self.hbar**2 / self.mass) * real_xx
        H_psi_imag = -0.5 * (self.hbar**2 / self.mass) * imag_xx

        # Residuales de las ecuaciones reales e imaginarias

        # residual_real = (H_psi_real / (-self.hbar * imag_t + eps) - 1) * (
        #    self.Xcar**2 / self.Tcar
        # )
        # residual_imag = (H_psi_imag / (self.hbar * real_t + eps) - 1) * (
        #    self.Xcar**2 / self.Tcar
        # )
